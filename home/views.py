...
def makePrediction (request):
    ...
    if request.method== 'POST' and 'predictors' in request.POST:
        ...
        clf = load(model)
        theModel= str(request.POST.get('theModel')).replace('.joblib','.txt')
        atts= ('.\static\\predictors\\'+ theModel)
        attributs= open(atts)
        lines= attributs.read()
        lines= lines.split(', ')
        print(lines)
    ...
    if request.method == 'POST' and 'train' in request.POST:
        ...
        datetime_str = datetime.datetime.now().strftime("(%H-%M--%d-%m-%Y)")
        dump(clf, '.\static\\predictors_test\\clf_classifier'+datetime_str+'.joblib')
        attFile = open('.\static\\predictors_test\\clf_classifier'+datetime_str+'.txt', "w")
        ...
        data = Accident.objects.all()
        myfilter = intervalledate2(prefix='pred')
        intervallePred = intervalledate3(prefix='makePred')
    ...
    if request.method == 'POST' and "savePredictor" in request.POST:
        list_of_files = glob.glob('.\static\\predictors_test\\*.joblib')
        list_of_atts = glob.glob('.\static\\predictors_test\\*.txt')
        clf = max(list_of_files, key=os.path.getctime)
        att = max(list_of_atts, key=os.path.getctime)
        print(clf)
        shutil.copy(clf, '.\static\\predictors\\',follow_symlinks=True)
        shutil.copy(att, '.\static\\predictors\\',follow_symlinks=True)
        # file_name = default_storage.save(clf, clf)
        # clf = load('.\static\\predictors\\clf_classifier.joblib')
        # dump(clf, clf)
    context = {'my_map': m, 'predictions':predictions, 'mars':res,
               'myfilter':myfilter, 'acc':acc, 'precision': precision, 'recall': recall,
               'f1score':f1score, 'roc':roc,'intervallePred': intervallePred, 'list_pred': list_pred}
